...
void enter(struct ST * table, char * name, int datatype, int offset);
...
void enter(struct ST * table, char * name, int datatype, int offset){
    ...
    table->SymbolTableBody[pos]->name = name;
    table->SymbolTableBody[pos]->datatype = datatype;
...
int gettoken(char *str) {
    ...
    if (SymbolTableBody[pos]->type != IDENTIFIER && SymbolTableBody[pos]->type != -1) {
        return SymbolTableBody[pos]->type;
    }
    return IDENTIFIER;
}
